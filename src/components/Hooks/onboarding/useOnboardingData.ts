    ...
    (async () => {
      try {
        ...
        if (!cfg || !cfg.components?.length) {
          console.warn(`Step ${stepNumber} has no components configured, redirecting...`, {
            config: cfg,
            hasComponents: cfg?.components?.length || 0
          });
          setRedirecting(true);
        ...
        if (profileRes.success && profileRes.data && typeof profileRes.data === 'object') {
          ...
          // Normalize camelCase field names to lowercase to match component names
          const normalizedData: Record<string, unknown> = {};
          Object.entries(profileRes.data).forEach(([key, value]) => {
            // Convert camelCase to lowercase for known fields
            let normalizedKey = key;
            if (key === 'aboutMe') normalizedKey = 'aboutme';
            if (key === 'birthDate') normalizedKey = 'birthdate';
